...
function isValidReading(body) {
  const { turbidity, tds, temperature, level } = body;
  return [turbidity, tds, temperature, level].every((v) => typeof v === 'number' && !Number.isNaN(v));
}
...
function postReading(req, res) {
  ...
  const { turbidity, tds, temperature, level, deviceId } = req.body;
  const reading = {
